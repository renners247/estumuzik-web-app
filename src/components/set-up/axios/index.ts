...
export default function setupAxios(axios: AxiosStatic, store: any) {
  ...
  axios.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      const errorDataMessage: any = error.response?.data;
      ...
      if (
        ...
        errorDataMessage?.message?.includes("Unauthenticated") ||
        errorDataMessage?.message?.includes("User subscription has expired")
      ) {
